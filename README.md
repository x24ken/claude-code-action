...
jobs:
  claude-response:
    ...
    steps:
      - uses: x24ken/claude-code-action@beta  # Fork with OAuth support
        with:
...
steps:
  - uses: x24ken/claude-code-action@beta
    with:
      direct_prompt: |
        Update the API documentation in README.md to reflect
        the changes made to the API endpoints in this PR.
...
This action is built on top of [`x24ken/claude-code-base-action`](https://github.com/x24ken/claude-code-base-action).
